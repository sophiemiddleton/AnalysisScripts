...
def compare_datasets( files, cuts, locations, columns, signs):
    ...
    # plot overlayes
    comparison.fit_momentum(recomom)
    #comparison.plot_particle_counts(mc_count, columns)

    #comparison.overlay_fit(0.47478,0.0159684, recomom, mc_count) #0.667162,-0.418574,-0.173891,0.0588692,0.0232978
    ...
    comparison.plot_resolution(resolutions, "True - Reco at TrkEnt [MeV/c]","recomom", -10,3, columns, density=True)
...
def  main(args):
  ...
  new = [True, True, False, True, True,False, True, True, True, True,False, True,True, False]
  ...
  # + and -
  signs = ["minus","minus"]
  files = ["OnSpillAll.txt","AllOffSpill.txt"]
  locations = [args.loc,args.loc]
  columns = ["on-spill","off-spill"]
